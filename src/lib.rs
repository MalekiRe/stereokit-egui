...
use egui::{ClippedPrimitive, Context, ecolor, emath, epaint, Event, Id, ImageData, PaintCallback, PlatformOutput, PointerButton, Pos2, RawInput, Response, Sense, Stroke, Style, TextureId, TouchPhase, Ui, Widget};
...
impl Widget for SkButton {
    fn ui(self, ui: &mut Ui) -> Response {
        ...
        let text_size: Vec2 = unsafe {
            stereokit_sys::text_size(c_str.as_ptr(), stereokit_sys::ui_get_text_style())
        }.into();
...
pub fn main() {
    ...
    let shader = sk.shader_create_mem(SHADER_FILE).unwrap();
    let material = sk.material_create(shader);
    sk.run(|sk| {

        sk.mesh_draw(stereokit::Mesh::CUBE, &material, Mat4::default(), Color128::new_rgb(1.0, 1.0, 0.0), RenderLayer::LAYER_ALL);
        /*
